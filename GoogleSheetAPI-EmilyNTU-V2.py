...
def fetch_all_instagram_media_ids(user_id: str, access_token: str, api_version: str = 'v24.0') -> list:
    ...
    # 2. 將字串轉換為 datetime 物件 (包含時區資訊)
    # %z 會解析 +0000 這種時區格式

    # 3. 取得現在的系統時間 (務必同樣使用 UTC 時區，以保證比較基準一致)
    now = datetime.now(timezone.utc)

    # 4. 計算一年前的時間點
    three_months_ago = now - relativedelta(months=12)
    ...
    while True:
        try:
            ...
            for media in current_page_data:
                ...
                ts_string = media.get('timestamp')
                dt_object = datetime.strptime(ts_string, '%Y-%m-%dT%H:%M:%S%z')
                if dt_object < three_months_ago:
                  pass_media_list.append(media.get('id'))
    ...
    return all_media_ids, pass_media_list
...
if __name__ == '__main__':
    if INSTAGRAM_USER_ID == 25544294341841753:
        ...
        all_ids, PASS_LIST = fetch_all_instagram_media_ids(INSTAGRAM_USER_ID, ACCESS_TOKEN)
...
HEADERS_MEDIA = ['media_id','write_date','comments_count','is_shared_to_feed','like_count','media_type',
          'media_url','permalink','shortcode','timestamp','caption']
...
if worksheet_media is None:
    ...
    for media_id in all_ids:
        ...
        media_params = {
            'fields': 'comments_count,is_shared_to_feed,like_count,media_type,media_url,permalink,shortcode,timestamp,caption',
            'access_token': ACCESS_TOKEN
        }
